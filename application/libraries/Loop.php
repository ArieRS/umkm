            ...
            <div class="product-list <?= $carousel == true ? 'item' : '' ?> <?= $classes ?> <?= $active ?>">
                <div class="inner">
                    <div class="img-container">
                        <a href="<?= $article['vendor_address'] == null ? LANG_URL . '/' . $article['url'] : LANG_URL . '/' . $article['url'] ?>">
                            <img src="<?= base_url('/attachments/shop_images/' . $article['image']) ?>"
                                 alt="<?= str_replace('"', "'", $article['title']) ?>">
                        </a>
                    </div>
                    <h2>
                        <a href="<?= $article['vendor_address'] == null ? LANG_URL . '/' . $article['url'] : LANG_URL . '/' . $article['url'] ?>"><?= character_limiter($article['title'], 70) ?></a>
                    </h2>
                    <div class="price">
                        <span class="underline"><?= lang('price') ?>
                            : <span><?= CURRENCY.' ' ?><?= $article['price'] != '' ? number_format($article['price'], 2) : 0 ?></span></span>
                        <?php
                    ...
                    <div class="price-discount <?= $article['old_price'] == '' ? 'invisible' : '' ?>">
                        <?= lang('old_price') ?>:
                        <span><?= CURRENCY.' '. ($article['old_price'] != '' ? number_format($article['old_price'], 2) : '') ?></span>
                    </div>
                    ...
                    if (self::$CI->load->get_var('moreInfoBtn') == 1) { ?>
                        <a href="<?= $article['vendor_address'] == null ? LANG_URL . '/' . $article['url'] : LANG_URL . '/' . $article['vendor_address'] . '/' . $article['url'] ?>"
                           class="info-btn gradient-color">
                            <span class="text-to-bg"><?= lang('info_product_list') ?></span>
                        </a>
                    <?php } ?>
                    <?php
                    if ((isset($_SESSION['logged_vendor']) == true) && ($_SESSION['logged_vendor'] == $article['email'])) {
//                        //tidak tampil button untuk beli produk sendiri
                    } else { ?>
