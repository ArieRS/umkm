...
class VerifiedProducts extends ADMIN_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('Verified_Products_model', 'Languages_model', 'Categories_model'));
    }
    ...
    public function index($page = 0)
    {
        ...
        if (isset($_GET['delete'])) {
            $this->Verified_Products_model->deleteProduct($_GET['delete']);
        ...
        $rowscount = $this->Verified_Products_model->productsCount($search_title, $category);
        $data['products'] = $this->Verified_Products_model->getproducts($this->num_rows, $page, $search_title, $orderby, $category, $vendor);
        $data['links_pagination'] = pagination('admin/products', $rowscount, $this->num_rows, 3);
        $data['num_shop_art'] = $this->Verified_Products_model->numShopproducts();
        $data['languages'] = $this->Languages_model->getLanguages();
        $data['shop_categories'] = $this->Categories_model->getShopCategories(null, null, 2);
        $this->saveHistory('Go to products');
        $this->load->view('_parts/header', $head);
        $this->load->view('ecommerce/verified_products', $data);
    ...
    {
        $this->login_check();
        return $this->Verified_Products_model->getOneProduct($id);
    }
    ...
    {
        $this->login_check();
        $result = $this->Verified_Products_model->productStatusChange($_POST['id'], $_POST['to_status']);
        if ($result == true) {
            echo 1;
        } else {
            echo 0;
